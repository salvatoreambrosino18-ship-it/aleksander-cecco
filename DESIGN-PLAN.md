...
design. Regenerate PNGs with scratchpad/icons.mjs, which proves the pixel
size, and LOOK at the output.
...
- Both PNGs regenerated by `scratchpad/icons.mjs`, which sets the viewport to
  EXACTLY the icon size at DPR 1 (so the screenshot *is* the icon) and decodes
